        ...
        <h1 class="pull-right">
            ...
            @if(Auth::user()->id == $account->user_id && $account->applied_for_payout != 1 )
                ...
                <input type="hidden" value="{{ $account->id}}" name="apply_for_payout">
                   {!! Form::button('<i class="fa fa-credit-card"></i> Apply for the payout', ['type' => 'submit', 'class' => 'btn btn-success', 'onclick' => "return confirm('Are you sure you wish apply for the payout?')"]) !!}
                {!! Form::close() !!}
            @endif

                @if(Auth::user()->role_id ==1 && $account->paid != 1)
                ...
                <input type="hidden" value="{{ $account->id}}" name="mark_as_paid">
                   {!! Form::button('<i class="glyphicon glyphicon-ok"></i> Mark as paid', ['type' => 'submit', 'class' => 'btn btn-success', 'onclick' => "return confirm('Do you want cofirm the payout?')"]) !!}
                {!! Form::close() !!}
                @endif
        </h1>
    </section>
